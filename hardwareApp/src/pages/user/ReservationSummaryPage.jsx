...
function ReservationSummaryPage() {
  ...
  const [items, setItems] = useState(ReservationService.getItems());
  ...
  const updateQty = (id, qty) => ReservationService.updateQuantity(id, qty);
  const removeItem = (id) => ReservationService.removeItem(id);

  const subtotal = ReservationService.getSubtotal();
    ...
    <Container maxWidth="md">
      ...
      {items.length === 0 ? (
        <Box textAlign="center" sx={{ py: 6 }}>
          ...
          <Button variant="contained" onClick={() => navigate('/user')}>Browse Products</Button>
        </Box>
      ) : (
        <Box>
          {items.map((item) => (
          ...
          <Paper elevation={2} sx={{ p: 3, mt: 3 }}>
            ...
            <Box display="flex" gap={2} sx={{ mb: 2 }}>
              <Button variant="outlined" onClick={() => navigate('/user')} sx={{ borderColor: '#d32f2f', color: '#d32f2f' }}>
                Continue Browsing
              </Button>
            </Box>

            <Button fullWidth variant="contained" size="large" startIcon={<ReceiptLong />} onClick={() => navigate('/user/reservation/receipt', { state: { items, subtotal, deliveryFee: 0, total: subtotal } })} sx={{ py: 1.5 }}>
              Confirm Reservation
            </Button>
          </Paper>
        </Box>
